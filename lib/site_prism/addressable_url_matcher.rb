...
module SitePrism
  class AddressableUrlMatcher
    ...
    def component_matches(component, uri)
      mappings = {}
      if (component_template = component_templates[component])
        ...
        unless mappings
          ...
          mappings = component_template.extract(prefix + component_url)
        end
      end
      mappings
    end
    ...
    def reverse_substitutions
      @reverse_substitutions ||=
        slugs.each_with_index.reduce({}) do |memo, slug_index|
          slug, index = slug_index
          memo.merge(slug_prefix(slug) + substitution_value(index) => slug,
